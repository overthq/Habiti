...
import type { GetProductResponse, Product as ProductType } from '@/data/types';
...
const StorePreview = () => {
	...
	return (
		<div className='mb-4 flex gap-2 items-center'>
			<Link to='/store/$id' params={{ id: product.store.id }}>
...
interface ProductContextProviderProps
	extends React.PropsWithChildren, GetProductResponse {}
			...
			try {
				if (!cartProduct) {
					...
					if (buyNow) {
						navigate({
							to: '/carts/$id',
							params: { id: newCartProduct.cartId }
						});
					}
				} else {
					...
					if (buyNow && cartId) {
						navigate({ to: '/carts/$id', params: { id: cartId } });
					}
				}
			} catch (error) {
				console.log(error);
